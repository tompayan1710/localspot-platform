...
export default function Today() {
  ...
  const { authState, logout } = useContext(AuthContext);
    ...
    try {
        ...
        setNext7Days(days);
        const res = await fetch(`${process.env.REACT_APP_API_URL}/api/reservations/getnext7days`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ provider_id, days }),
        });
  ...
  useEffect(() => {
    if(!isLoading) {
        setCreneauOfSelectedDate(() => {
            ...
            allCreneaux[next7Days[selectedDay].toLocaleDateString('fr-CA')].forEach((slot) => {
                // console.log(slot);
            ...
            <div className="barToScroll"></div>
            {
                ...
                (
                    ...
                    Object.entries(creneauOfSelectedDate).map(([timeRange, slotsArray]) => {
                        ...
                        const slot_end_hour = slot_hours[0];
                        ...
                        return (
                            <div key={timeRange} className={`SlotReservation ${slotStatus === "completed" && "completed"}`}>
                                ...
                                <div className="row">
                                    <div className={`${slotStatus} column CreneauContainer`}>
                                ...
                                {slotsArray.map((slot) => (
                                    <div key={`${slot.start_hour}-${slot.end_hour}`} className={`ReservationItem ${slotStatus === "completed" && "completed"}`}>
                                        {slotStatusLabels[slotStatus] && (
                                            // <p className="t5">{t(`status.${slotStatusLabels[slotStatus]}`)}</p>//Plus tard avec i18n
                                            <p className="t5">{`${slotStatusLabels[slotStatus]}`}</p>
                                        )}
                                        ...
                                        <div className="row">
                                            <div className={`${slotStatus} column CreneauContainer`}>
                                                <div className="t6 Creneau">{slot.start_hour}</div> 
                                                <div className="lineHorrizontal"></div>
                                                <div className="t6 Creneau">{slot.end_hour}</div> 
                                            </div>
